...
type SectionCategory = 'project-context' | 'decision' | 'architecture' | 'deviation' | 'security' | 'bug' | 'worklog' | 'conflict';
...
function summarizeBucket(title: string, entries: SearchResult[], category: SectionCategory, maxItems: number, labelPrefix: string): string {
...
function conflictWarnings(entries: SearchResult[]): string[] {
    ...
    for (const entry of entries) {
        ...
        if (text.includes('deprecated') || text.includes('superseded') || text.includes('needs review')) {
            warnings.push(
                bullet('C', `Potentially stale memory surfaced from ${entry.section_heading ?? path.basename(entry.source_path)}`, entry.source_path)
            );
        }
    }
...
export async function generateMemorySynthesis(
        ...
        formatItems(
            ...
            limitLines(
                results
                    ...
                    .map((entry, index) =>
                        bullet(`C${index + 1}`, entry.content_summary || entry.snippet || entry.section_heading || entry.source_path, entry.source_path)
                    ),
                3
            )
        )
    ...
    sections.push(formatItems('Conflict Warnings', limitLines(conflictWarnings(results), 3)));
    sections.push(
