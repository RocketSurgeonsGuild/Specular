...
export function scoreResult(result: SearchResult, query: string): SearchResult {
    ...
    const haystack = [result.source_path, result.section_heading ?? '', result.content_summary ?? '', result.snippet ?? '', result.tags ?? ''].join(' ');
...
export function searchMemoryEntries(db: MemoryDatabase, query: string, config: MemoryHubConfig, limit = config.retrieval.max_memory_results): SearchResult[] {
    ...
    return [...deduped.values()].sort((a, b) => b.score - a.score || (b.fts_rank ?? 0) - (a.fts_rank ?? 0)).slice(0, limit);
}
