...
export function parseMarkdownFile(filePath: string, rawContent: string): ParsedChunk[] {
    ...
    const flush = (endLine: number) => {
        ...
        for (let index = 0; index < sectionBlocks.length; index += 1) {
            ...
            const tags = uniqueSorted([fileStem, ...currentHeadingPath, ...headingWords(currentHeadingPath), ...extractTopicWords(summary, textChunk)]);

            const chunkLineStart = currentStartLine;
            const chunkLineEnd = endLine;
            const chunkHash = sha256([filePath, chunkHeading, chunkLineStart, chunkLineEnd, rawChunk, index].join('\n---\n'));
