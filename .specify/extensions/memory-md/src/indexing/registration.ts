 ...
 */
async function appendEntryToSourceFile(filePath: string, options: RegisterMemoryOptions): Promise<void> {
...
export async function registerMemoryEntry(projectRoot: string, db: MemoryDatabase, config: MemoryHubConfig, options: RegisterMemoryOptions): Promise<void> {
    ...
    if (options.content) {
        const targetFilePath = path.isAbsolute(options.file) ? options.file : path.join(memoryRoot, options.file);
