...
async function runTokenReport(projectRoot: string, featurePath: string): Promise<void> {
    ...
    try {
        ...
        const indexTokens = estimateTokens(baselineSources.find(file => path.basename(file.path) === 'INDEX.md')?.content ?? '');
...
export async function runCli(argv = process.argv): Promise<void> {
    ...
    program
        ...
        .option('--prepend', 'Insert entry before the first existing ### entry instead of appending at EOF. Use for WORKLOG (newest-first convention).')
