...
// Specular.Samples.Web — proves Specular's COMPILE-TIME scanning in a minimal-API host (US4).
//
...
if (missing.Count != 0 || unexpected.Count != 0)
{
    ...
    Console.Error.WriteLine("MISMATCH: Specular discovered service set does not equal the expected set.");
...
app.MapGet("/", () => $"Specular.Samples.Web — discovered {discovered.Count} services. See /services.");

Console.WriteLine($"Specular.Samples.Web: discovered {discovered.Count} services; opt-out type absent. Serving.");
app.Run();
...
static bool DescribesSample(Type serviceType) =>
    serviceType.Namespace is { } ns && ns.StartsWith("Specular.Samples.", StringComparison.Ordinal);
