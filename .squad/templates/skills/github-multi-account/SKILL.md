...
function gh-personal {
    gh auth switch --user $personal 2>$null | Out-Null
    gh @args
}

function gh-work {
    gh auth switch --user $work 2>$null | Out-Null
    gh @args
}
...
```powershell
function gh-personal {
    gh auth switch --user bradygaster 2>$null | Out-Null
    gh @args
}

function gh-work {
    gh auth switch --user bradyg_microsoft 2>$null | Out-Null
    gh @args
}
```
