...
internal sealed class SpecularScanReport
{
    ...
    , 
        [
            new("TestProject", "z => z.FromAssemblies().AddClasses(x => x.AssignableTo(typeof(IService)).AssignableTo<IServiceB>().EndsWith(\"Factory\")).AsSelf().AsImplementedInterfaces().WithScopedLifetime()", 
                [
                    new("TestProject", 
                        [
