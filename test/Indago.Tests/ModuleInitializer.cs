...
internal static partial class ModuleInitializer
{
    ...
    {
        FileExtensions.AddTextExtension(Path.GetExtension(Constants.IndagoProviderCacheFileName));
        ...
        VerifierSettings.ScrubLinesWithReplace(
            s => s.Contains("<Compiled_AssemblyProvider_g>", StringComparison.OrdinalIgnoreCase)
                ? s[..( s.IndexOf('"') + 1 )] + "{IndagoProvider}" + s[s.LastIndexOf('"')..]
                : s
        );
        ...
        VerifierSettings.AddScrubber(
            (builder, counter) =>
            {
                if (typeof(IndagoProviderGenerator).Assembly.GetCustomAttribute<AssemblyFileVersionAttribute>() is
                    { Version: { Length: > 0 } version })
                {
                    _ = builder.Replace(version, "version");
                }

                if (typeof(IndagoProviderGenerator).Assembly.GetCustomAttribute<AssemblyVersionAttribute>() is { Version: { Length: > 0 } version2 }) _ = builder.Replace(version2, "version");
    ...
    private static ConversionResult Convert(GeneratorTestResultsWithCacheFiles result, IReadOnlyDictionary<string, object> context)
    {
        ...
        var generatedCache = Directory
                            .EnumerateFiles(result.TempDirectory, Constants.IndagoProviderCacheFileName, SearchOption.AllDirectories)
                            .Select(
