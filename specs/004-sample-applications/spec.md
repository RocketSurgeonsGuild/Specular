...
- Q: What is the MAUI target-platform scope in CI for the AOT/trim publish pipeline? → A: The MAUI AOT/trim publish pipeline attempts a single CI-friendly MAUI platform, `net10.0-android`, as the genuinely-exercised target (publish must complete with no Specular-attributable trim/AOT warnings). The iOS, MacCatalyst, and Windows MAUI targets are explicitly skip-with-reason. This preserves the "skipped ≠ passed" guarantee while still proving Specular survives MAUI trimming on at least one real platform.
...
- Q: How should Specular-attributable AOT/trim warnings be distinguished from third-party ones so the build fails reliably? → A: Zero-warning policy. The demonstration hosts (Console/Web/Blazor-WASM/MAUI-android) must publish with ZERO trim/AOT warnings of any kind — each host's own publish closure is required-to-be-clean, so ANY trim/AOT warning (Specular or third-party) fails the build (warnings-as-errors for the publish). The dedicated negative-fixture host is the ONLY project permitted to warn, and its inverted-assertion step requires it to warn. This constrains the sample closure: demo libraries/hosts must keep their dependency closure AOT/trim-clean (limits what third-party packages they may pull in).
...
### User Story 1 - Reusable sample libraries demonstrating Specular scanning patterns (Priority: P1)

As an Specular maintainer and documentation author, I want a small set of shared, general-purpose sample libraries whose services are discovered by Specular's compile-time scanning, so I have realistic, reusable building blocks that prove scanning works and can be referenced from documentation.

**Why this priority**: The shared libraries are the foundation every host application consumes. Without them, no host can demonstrate scanning. They are the smallest slice that, on its own, proves the core promise of Specular (compile-time discovery of services) in a realistic multi-library arrangement.

**Independent Test**: Build the sample libraries together with a minimal consumer, then assert that Specular's generated provider registers exactly the expected set of services from each library, with the expected service/implementation mappings and lifetimes — without any runtime reflection.

**Acceptance Scenarios**:

1. **Given** three sample libraries each exposing between 3 and 6 services, **When** a consumer scans them via Specular fluent selectors (e.g. `FromAssemblyOf<>().AddClasses().AsMatchingInterface()`), **Then** every intended service from every library is registered and resolvable, and no unintended types are registered.
...
As an Specular maintainer, I want a Console host application that consumes the sample libraries, runs Specular scanning, and can be published with Native AOT, so I have the simplest possible end-to-end demonstration that scanning survives AOT/trimming.
...
1. **Given** the console host references the three sample libraries, **When** it runs, **Then** it demonstrates that the expected services are discovered and resolved through the Specular-generated provider and exits successfully.
...
3. **Given** `ISpecularProvider.EntryAssembly`-based access, **When** the host queries the entry assembly's discovered types, **Then** the result matches the expected set.

---

### User Story 3 - AOT publish of samples is exercised as a build pipeline test (Priority: P1)

As an Specular maintainer, I want the AOT publish of the sample hosts to be executed and verified as a step in the ModularPipelines build (`mise run build`), so regressions in AOT/trim compatibility are caught automatically in CI rather than discovered manually.
...
2. **Given** a sample host that fails to AOT-publish or emits any trim/AOT warning (Specular-attributable or third-party), **When** the pipeline runs, **Then** the pipeline step fails and the overall build fails.
...
As a documentation author, I want a minimal-API web host that uses Specular scanning and supports AOT publishing, so the docs can show how Specular integrates with the most common web hosting style.
...
As a documentation author, I want a Blazor WebAssembly host that uses Specular scanning and is published with WebAssembly AOT, so the docs cover client-side browser scenarios.

**Why this priority**: Blazor WASM showcases Specular in a trimming-heavy, browser-hosted environment, but it is lower priority than server and console hosts because of its heavier toolchain and longer publish times.

**Independent Test**: Load the Blazor WASM host (in a browser or via its published output) and confirm it asserts the expected service set and surfaces a detectable failure state on any mismatch, with a page/component listing the Specular-discovered services from the sample libraries; confirm the WebAssembly AOT publish completes successfully with zero trim/AOT warnings of any kind. The ModularPipelines run step treats the host's surfaced pass/fail state as the pass/fail signal.

**Acceptance Scenarios**:

1. **Given** the Blazor WASM host references the sample libraries, **When** it loads, **Then** a component displays the expected set of Specular-discovered services.
...
As a documentation author, I want a MAUI host that uses Specular scanning and exercises the platform's AOT/trimming publish path, so the docs cover cross-platform native application scenarios.
...
**Independent Test**: Launch the MAUI host on a supported platform and confirm it asserts the expected service set and surfaces a detectable failure state on any mismatch, with a screen listing the Specular-discovered services; confirm the `net10.0-android` AOT/trimmed publish completes with zero trim/AOT warnings of any kind, while iOS/MacCatalyst/Windows targets are reported as skip-with-reason. The ModularPipelines run step treats the host's surfaced pass/fail state as the pass/fail signal.

**Acceptance Scenarios**:

1. **Given** the MAUI host references the sample libraries, **When** it launches, **Then** the UI displays the expected set of Specular-discovered services.
...
1. **Given** a clean checkout, **When** `mise run build` runs, **Then** a named docs module generates the API reference from `src/Specular/bin/Release/net8.0/Specular.dll` + `Specular.xml` and builds the Starlight site into `artifacts/docs`.
2. **Given** the docs module ran, **When** the build completes, **Then** no separate `dotnet publish` of Specular for docs occurred — the API reference was generated from the normal Release build output.
...
- **Negative-fixture regression guard**: The dedicated negative-fixture host must continue to emit its Specular-attributable trim/AOT warning. If a future change causes that warning to disappear (e.g. the fixture stops triggering the regression path), the inverted-assertion step must fail the build, signalling that the regression detector can no longer prove failures are caught—rather than silently passing.
- **Clean dependency closure constraint (zero-warning policy)**: Because each demonstration host's AOT publish must be entirely warning-free (any trim/AOT warning, Specular or third-party, fails the build), the sample libraries and hosts must keep their full dependency closure AOT/trim-clean. This constrains which third-party packages the demo libraries/hosts may reference—any package that emits trim/AOT warnings under the target publish would break the build and must be avoided, replaced, or have its warnings legitimately resolved. The negative-fixture host is the sole exception (it is required to warn).
- **Docs build consumes bin, not publish**: The API reference is generated from the project's Release build output (`src/Specular/bin/Release/net8.0/`), so the docs module depends on a prior Release build producing both `Specular.dll` and `Specular.xml` (XML documentation generation enabled). If the XML doc file is missing, the docs module MUST fail clearly rather than emit an empty or partial API reference.
...
- **FR-002**: Each sample library MUST expose between 3 and 6 services (service/implementation pairs) intended to be discovered by Specular.
- **FR-003**: The three libraries together MUST illustrate more than one Specular registration style (at minimum: interface-matching registration, attribute-based registration, and at least two distinct service lifetimes).
...
- **FR-007**: Each host application MUST consume the shared sample libraries and use Specular's compile-time scanning to discover and register their services (no runtime reflection-based scanning).
...
- **FR-009**: Each host application MUST be a minimal demonstration focused solely on showcasing Specular scanning and AOT compatibility, avoiding unrelated application complexity.
...
- **FR-012**: An AOT/trimming publish of any demonstration host (Console, Web, Blazor WASM, MAUI-android) MUST complete with ZERO trim or AOT warnings of any kind — its own publish closure is required-to-be-clean. ANY trim/AOT warning (whether attributable to Specular, its generated output, or a third-party dependency) MUST be treated as a build failure (warnings-as-errors for the publish). The dedicated negative-fixture host (FR-020/FR-021) is the sole exception and is excluded from this rule.
...
- **FR-020**: The feature MUST provide a permanent dedicated negative-fixture host under `samples/` whose sole purpose is to intentionally emit an Specular-attributable trim/AOT warning during AOT publish. This fixture MUST be excluded from the "normal" AOT-publish steps and MUST NOT be treated as one of the four demonstration host applications.
- **FR-021**: The pipeline MUST AOT-publish the negative-fixture host as a discrete, named step using the same AOT publish target as the demonstration hosts (latest TFM `net10.0` × the current-OS RID) and an inverted assertion: the step MUST pass only when that publish warns/fails (i.e. the expected Specular-attributable trim/AOT warning is present), and MUST fail the build if the publish ever becomes clean or the expected warning stops appearing—so the regression detector itself is guarded against silently breaking. The negative-fixture host is the ONLY project exempt from the zero-warning policy (FR-012/FR-015) and is in fact required to warn.

#### Documentation

- **FR-018**: The samples MUST be referenceable from the project documentation as worked examples of Specular usage across host types, consistent with the project's documentation-as-deliverable principle.
- **FR-019**: Each host sample MUST make its Specular usage (the selector expressions and registration approach) easy to extract for documentation, without requiring the reader to run the sample.
...
- **Sample Service**: A service/implementation pair within a sample library that Specular is expected to discover, with a defined lifetime and registration style; some are explicitly excluded to demonstrate opt-out.
- **Host Application**: A runnable demonstration (Console, Web minimal API, Blazor WASM, or MAUI) that consumes the sample libraries, performs Specular scanning, and exposes a visible discovery result.
...
- **SC-002**: All four host applications (Console, Web minimal API, Blazor WASM, MAUI) build, run, and visibly demonstrate the correct, complete set of Specular-discovered services from every sample library; each host asserts the expected service set in-process and signals mismatch via a nonzero exit code (Console/Web at startup) or a detectable failure state (Blazor WASM/MAUI), which the ModularPipelines run step interprets as the host's pass/fail outcome.
- **SC-003**: Running the ModularPipelines build executes the AOT publish validation as discrete, named step(s), and a permanent dedicated negative-fixture host—one that intentionally emits an Specular-attributable trim/AOT warning—is validated with an inverted assertion (its AOT publish step passes only when that publish warns/fails as expected). If the expected warning ever stops appearing, the negative-fixture step and the overall build fail. The negative-fixture host is excluded from the "normal" AOT-publish steps and is the only project exempt from the zero-warning policy.
- **SC-004**: For every demonstration host, an AOT/trim publish completes for the AOT publish target (latest TFM `net10.0` × the current-OS RID) with ZERO trim/AOT warnings of any kind (Specular-attributable or third-party); any warning fails the build.
...
- **SC-007**: Each host sample's Specular usage is documented (or directly referenceable) so a reader can understand the scanning approach without executing the sample.
...
- "Scrutor assembly scanning" in the user description refers to Specular's Scrutor-style compile-time assembly scanning; Specular is the AOT-friendly replacement for runtime reflection-based scanning, and the samples demonstrate Specular's scanning.
...
- CI environments may lack some workloads (e.g. MAUI per-platform workloads, `wasm-tools`); the pipeline is expected to skip-with-reason in those cases rather than fail, while still failing on genuine Specular AOT regressions where the toolchain is present.
...
- The API reference is generated from the project's normal Release build output (`src/Specular/bin/Release/net8.0/Specular.dll` + `Specular.xml`), which assumes XML documentation generation is enabled for the Specular project; no separate publish is performed for docs. (Verified: the Release bin already produces `Specular.xml`.)
