...
- Registration styles:
    ...
    - **Attribute-based**: types decorated with `[ServiceRegistration(lifetime, ...)]` /
      ...
      `services.AddIndagoServiceRegistrations(provider)` scans for them (registers `AsSelf`,
...
## DEVIATION D-OPTOUT — `[ExcludeFromIndago]` is assembly-only

The contract/tasks call for "exactly one **type** marked `[ExcludeFromIndago]`". But
`Indago.Abstractions.ExcludeFromIndagoAttribute` is declared `[AttributeUsage(AttributeTargets.Assembly)]`
...
- `samples/Directory.Build.props` (IsPackable=false, RS0017 dropped), `Directory.Build.workloads.props`
  (D1 guards: `IndagoSamplesMauiEnabled`, `IndagoSamplesWasmAotEnabled` default false), and
  `samples/Directory.Build.targets` (wires the Indago analyzer via `UsesIndagoGenerator=true`).
- Root `Directory.Packages.props` already has DI, AspNetCore.Components.WebAssembly, Playwright (T004).
- `Indago.slnx` has the `/samples/` folder with the two props files.

## PRE-EXISTING RED TEST BASELINE (not caused by this feature)

`dotnet test test/Indago.Tests` is **376/376 failing** — Verify snapshot drift. Cause: the
...
- `IIndagoProvider.GetTypes` selector uses `IReflectionTypeSelector` (terminal `.GetTypes(filter)`),
  NOT the `.AddClasses().AsMatchingInterface()` service-descriptor chain.

### NOTE — possible bug in `samples/Directory.Build.targets`

`AdditionalFiles Include` points at `$(IntermediateOutputPath)\ctp\IndagoProvider.ctpjson` but its
`Condition` checks `Exists('...\gap\IndagoProvider.ctpjson')` (`ctp` vs `gap` mismatch). Revisit if a
