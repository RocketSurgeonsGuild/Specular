# Tasks: Specular VitePress Documentation Site

**Input**: Design documents from `specs/001-specular-static-assembly-scanner/`
...
- [ ] T005 Create `oxlintrc.json` at repo root per the contract in `specs/001-specular-static-assembly-scanner/contracts/vitepress-config.md` (oxlint contract section) — set `plugins: ["vue", "typescript"]`, `ignorePatterns` for `node_modules`, `docs/.vitepress/dist`, `docs/.vitepress/cache`, `apm_modules`, `.apm`
...
```bash
mise run docs
# Navigate to http://localhost:5173/Specular/
# Verify: title "Specular", nav shows Guide / Reference / Architecture / GitHub, sidebar shows all sections
```

- [ ] T007 [US3] Rewrite `docs/.vitepress/config.mts` per the VitePress contract in `specs/001-specular-static-assembly-scanner/contracts/vitepress-config.md`: set `title: "Specular"`, `description: "Compile-time assembly scanning for .NET — AOT safe, zero reflection"`, `lang: "en-US"`, `base: "/Specular/"`, `cleanUrls: true`, `lastUpdated: true`, nav with Guide/Reference/Architecture/GitHub/NuGet links, sidebar object with `/guide/`, `/reference/`, `/architecture/` sections per `data-model.md`, `editLink`, `socialLinks`, `search: { provider: "local" }`
...
- [ ] T009 [P] [US3] Rewrite `docs/index.md` as the home page: hero with `name: "Specular"`, `text: "Compile-time assembly scanning for .NET"`, `tagline: "AOT safe. Zero reflection. Build-time performance."`, two action buttons (brand: "Get Started" → `/guide/`, alt: "API Reference" → `/reference/ispecular-provider`), three feature cards: "AOT Safe" / "Build-Time Scanning" / "Minimal API Surface"
...
- [ ] T010 [P] [US3] Create `docs/guide/index.md` — "What is Specular?" page: explain the problem (runtime reflection cost, AOT incompatibility), the solution (Roslyn source generator emitting a strongly-typed provider), and the key concepts (`ISpecularProvider`, selector expressions, `SpecularProvider.ctpjson`). Read `src/Specular/ISpecularProvider.cs` for API shape. Include a one-paragraph architecture summary linking to `/architecture/how-it-works`.
- [ ] T011 [P] [US3] Create `docs/guide/installation.md` — NuGet install + project setup: `dotnet add package Specular`, wire `[assembly: SpecularProvider]` attribute, confirm generated file appears under `obj/`. Read `src/Specular/SpecularHashAttribute.cs` for attribute usage. Include a minimal working csproj snippet.
- [ ] T012 [P] [US3] Create `docs/guide/quickstart.md` — First selector walkthrough: call `provider.GetTypes(s => s.FromAssemblyOf<Program>().AddClasses().AsImplementedInterfaces())`, register results with `AddSpecularServiceRegistrations`, show the generated code structure. Read `src/Specular/ISpecularProvider.cs` and `src/Specular/SpecularProviderServiceCollectionExtensions.cs`.
- [ ] T013 [P] [US3] Create `docs/guide/aot-publishing.md` — AOT/trim compatibility guide: `dotnet publish -r <rid> --self-contained -p:PublishAot=true`, explain why zero reflection matters, note both `net8.0` and `net10.0` targets. Read `src/Specular/ISpecularProvider.cs` doc comments. End with "Next steps" linking to Reference section.
...
- [ ] T014 [P] [US3] Create `docs/reference/ispecular-provider.md` — `ISpecularProvider` API reference: document `GetAssemblies()`, `GetTypes()`, `Scan()`, `EntryAssembly`, `GetArgumentExpressionHash()`. Explain the `[CallerLineNumber]`/`[CallerFilePath]`/`[CallerArgumentExpression]` hidden parameters and why they matter. Read `src/Specular/ISpecularProvider.cs`.
- [ ] T015 [P] [US3] Create `docs/reference/type-filters.md` — Fluent selector API reference: document `IReflectionAssemblySelector`, `ITypeFilter`, `AddClasses()`, `AsImplementedInterfaces()`, `AsMatchingInterface()`, `AssignableTo<T>()`, `WithAttribute<T>()`, namespace scoping, type-kind filters. Read files in `src/Specular/Abstractions/`.
- [ ] T016 [P] [US3] Create `docs/reference/service-registration.md` — `ServiceRegistrationAttribute` and `RegistrationLifetimeAttribute` reference: purpose, constructors, `Singleton`/`Scoped`/`Transient` lifetime values, `AddSpecularServiceRegistrations` usage. Read `src/Specular/ServiceRegistrationAttribute.cs`, `src/Specular/RegistrationLifetimeAttribute.cs`, `src/Specular/SpecularProviderServiceCollectionExtensions.cs`.
- [ ] T017 [P] [US3] Create `docs/reference/exclude-from-specular.md` — `ExcludeFromSpecularAttribute` reference: when to use it, how it interacts with all three `ISpecularProvider` methods, an example. Read `src/Specular/ExcludeFromSpecularAttribute.cs`.
...
**Independent Test** (US3 Scenario 2): A developer navigating to Architecture finds a clear explanation of `SpecularProvider.ctpjson` and `GeneratedHash`.

- [ ] T018 [P] [US3] Create `docs/architecture/how-it-works.md` — Generator pipeline overview: syntax providers (`AssemblyCollection`, `ReflectionCollection`, `ServiceDescriptorCollection`), selector expression hashing (`GetArgumentExpressionHash`), symbol visitor flow in `AssemblyProviders/`, code emission. Include a simple flow diagram in Mermaid. Read `src/Specular.Analyzers/CompiledTypeProviderGenerator.cs` for the pipeline structure.
- [ ] T019 [P] [US3] Create `docs/architecture/cross-assembly-caching.md` — Cross-assembly caching model: what `SpecularProvider.ctpjson` contains, how `GeneratedHash` in `SpecularHashAttribute` is used for cache invalidation, the two-project (LibA → AppB) scenario, what happens when cache is missing or corrupt. Read `src/Specular.Analyzers/Configuration/` and `src/Specular/SpecularHashAttribute.cs`.
...
- [ ] T021 [P] Run Scenario 1 (dev server): `mise run docs` — verify home page renders at `http://localhost:5173/Specular/`, all nav links resolve, no console errors
...
Task: "Create docs/reference/ispecular-provider.md"
Task: "Create docs/reference/type-filters.md"
Task: "Create docs/reference/service-registration.md"
Task: "Create docs/reference/exclude-from-specular.md"
...
- Content pages (T010–T019) require reading the corresponding source files in `src/Specular/` and `src/Specular.Analyzers/` — file paths are listed in each task
