# Research: Specular VitePress Documentation Site
...
- `base`: Set to `/Specular/` for GitHub Pages project-site hosting (unless a custom domain maps
  to root). If a custom domain is added later, change `base` to `'/'`.
- `title` + `description`: "Specular" / "Compile-time assembly scanning for .NET"
...
- `themeConfig.editLink`: Points to `https://github.com/RocketSurgeonsGuild/Specular/edit/main/docs/:path`
...
| Reference    | 4     | API types (ISpecularProvider, type filters, ServiceRegistration, Exclude) |
...
Scrutor features **not** currently in Specular (candidates for future additions, not in this
docs scope):

| Scrutor Feature                                   | Specular Status | Notes                                                                      |
| ------------------------------------------------- | ------------- | -------------------------------------------------------------------------- |
...
| `AsSingletonOf<T>()` (lifetime per-registration)  | Partial       | Specular uses attribute-based lifetime only                                  |
