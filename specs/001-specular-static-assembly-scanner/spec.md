# Feature Specification: Specular — Static Assembly Scanner for .NET

**Feature Branch**: `001-specular-static-assembly-scanner`
...
**Why this priority**: This is the entire reason Specular exists. Without it there is no product.
...
2. **Given** a consumer adds the `[ExcludeFromSpecular]` attribute to a class,
...
3. **Given** a class is decorated with `[ServiceRegistration]` (default Singleton lifetime),
   **When** `AddSpecularServiceRegistrations(services)` is called,
...
(`SpecularProvider.ctpjson`) emitted by the upstream library's generator pass and uses it
...
1. **Given** LibA emits `SpecularProvider.ctpjson` during its build,
...
2. **Given** a type in LibA changes its implemented interfaces,
   ...
   **Then** AppB's generated scan results reflect the updated interface list (cache is
   invalidated by the `GeneratedHash` embedded in `SpecularHashAttribute`).

---

### User Story 3 — Public Documentation Site (Priority: P3)

A developer new to Specular visits the VitePress documentation site to understand how to add
Specular to their project, write their first type selector, and publish an AOT-compatible binary.
...
**Independent Test**: A developer who has never used Specular can follow the quickstart guide
...
1. **Given** a developer visits the docs home page on GitHub Pages,
   ...
   **Then** they can add the Specular NuGet package, write a selector, and verify the generated
...
2. **Given** a developer wants to understand the cross-assembly caching model,
   ...
   **Then** they find a clear explanation of `SpecularProvider.ctpjson`, `GeneratedHash`, and the
...
- What happens when `SpecularProvider.ctpjson` is missing or corrupt for a referenced assembly?
...
- **FR-001**: The library MUST provide a Roslyn incremental source generator that evaluates
  type-selector expressions at compile time and emits a concrete `ISpecularProvider` implementation
  ...
  type-kind filters (concrete, open-generic). Scrutor API gaps not yet covered by Specular are
...
- **FR-002**: The generator MUST support `GetAssemblies()`, `GetTypes()`, and `Scan()` on
  `ISpecularProvider`, each keyed by a stable hash of the selector argument expression at the
  call site.
...
- **FR-005**: The library MUST provide `[ExcludeFromSpecularAttribute]` to opt individual types
  out of all scan results.
- **FR-006**: The generator MUST serialize scan results to `SpecularProvider.ctpjson` using
  source-generated JSON serialization so downstream assemblies can reuse them without
  re-resolving.
- **FR-007**: The generator MUST read `SpecularProvider.ctpjson` from referenced assemblies
  (via `AdditionalText`) and use it as a cache, validated against the `GeneratedHash` embedded
  in `[SpecularHashAttribute]`.
- **FR-008**: The generator MUST compile and produce correct output on Roslyn 4.8, 4.14, and 5.0.
- **FR-009**: The library MUST expose `AddSpecularServiceRegistrations(IServiceCollection)` to
...
- **FR-010**: The documentation site MUST include a quickstart guide, conceptual architecture
  explanation, and API reference for all public types in `src/Specular`.
...
- **ISpecularProvider**: The generated provider interface; entry point for all scanning operations.
  Resolved at startup via `[SpecularHashAttribute]` on the entry assembly.
...
- **SpecularProvider.ctpjson**: The cross-assembly cache artifact; contains serialized scan
...
- **ServiceRegistrationAttribute**: Marks a class for DI auto-registration; carries optional
  lifetime override.
- **ExcludeFromSpecularAttribute**: Opts a type out of all Specular scan results.

## Success Criteria _(mandatory)_

### Measurable Outcomes

- **SC-001**: A .NET application using Specular for DI registration publishes successfully with
...
- **SC-003**: A developer new to Specular can reach a working, AOT-published "hello world"
...
- Q: What is the complete set of type filter/selector methods in scope for v1? → A: Class/interface
  ...
  yet supported by Specular are candidates for inclusion and should be identified during planning.
...
- The DI registration helper (`AddSpecularServiceRegistrations`) targets `Microsoft.Extensions.DependencyInjection.Abstractions`;
